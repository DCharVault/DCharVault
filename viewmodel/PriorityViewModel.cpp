...
bool PriorityViewModel::saveLabels(const QVariantList &labels)
{
    ...
    const DiaryError err = DiaryError::None; /*m_manager.saveConfig(QString::fromLatin1(kConfigKey), json);*/
...
QString PriorityViewModel::buildLabelHtml(const QString &name, const QString &color)
{
    ...
    // Simple heuristic: if color is dark use white text, else dark text
    QColor bg(color);
